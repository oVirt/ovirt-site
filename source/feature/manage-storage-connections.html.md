...
*   Add lock that locks the connection's id in UpdateStorageServerConnection and RemoveStorageServerConnection. (for cases where no storage domain exists, so the lock of domain's id is irrelevant). (in implementation)
