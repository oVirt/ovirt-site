...
**Network labels** feature provides the ability to label networks with one or more labels and to use the label(s) on the host's interfaces, so the label abstracts the networks from the physical interface/bond which can be labelled with one or more labels:
