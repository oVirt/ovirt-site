...
[id="appe-oVirt_and_Encrypted_Communication"]
// keeping old section id to prevent broken links
[id="appe-oVirt_and_SSL"]
...
* xref:enabling-fips_in_ovirt[Enable FIPS in oVirt].
// * Creating oVirt hosts that have FIPS enabled, or enabling FIPS on existing hosts.
// * Enabling FIPS using the oVirt Engine, including:
...
* xref:configuring_remote_viewer_trust_ovirt_ca_certificate[Configure the Remote Viewer to trust the Manager's CA certificate].
...
include::topics/Configuring_remote_viewer_trust_oVirt_ca_certificate.adoc[leveloffset=+2]

[role="_additional-resources-configuring-remote-viewer"]
.Additional resources

* link:{URL_virt_product_docs}{URL_format}virtual_machine_management_guide#sect-Installing_Console_Components[Installing Console Components]
