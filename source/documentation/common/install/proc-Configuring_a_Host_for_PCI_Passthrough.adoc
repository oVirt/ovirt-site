...
* To enable the IOMMU flag from the Administration Portal, see link:{URL_virt_product_docs}admin-guide/administration-guide.html#Adding_standard_hosts_to_the_Manager[Adding Standard Hosts to the {virt-product-fullname} {engine-name}] and link:{URL_virt_product_docs}admin-guide/administration-guide.html#Kernel_Settings_Explained[Kernel Settings Explained].
