...
5.  engine-setup on the first machine generates and signs also the cert for the WebSocketProxy one: having it running on a different host with a different fdqn requires an additional SSL cert

The engine-setup process will be updated asking (both in the interactive way and in the answer file) for the details (fqdn, port) of the machine that will run the WebSocketProxy.
