...
[NOTE]
====
If upgrading from version 4.4.9 to a later version fails on RHV-H, run the `yum reinstall redhat-virtualization-host-image-update` command to fix the issue.
====
