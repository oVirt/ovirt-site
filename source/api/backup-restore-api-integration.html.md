...
2. POST the copied disk with the disk id and the snapshot id: ([Example](Features/Backup-Restore_API_Integration#Detach_disk_from_backup_VM))
...
3. After copying the data from the disk detach the disk snapshot from the VM using the REST with the following parameters ([Example](Features/Backup-Restore_API_Integration#Attach_disk_to_backup_VM)):
