...
. If you want to use a custom appliance for the virtual machine installation, enter the path to the appliance image. The deployment supports both qcow2 (Kiwi-built appliances, version 4.5.1 and later) and OVA (traditional) formats. Otherwise, leave this field empty to use the {engine-appliance-name}.
+
[source,terminal,subs="normal"]
----
...
----
+
NOTE: For appliances version 4.5.1 and later, the Engine virtual machine automatically boots with UEFI. Older appliances continue to use legacy BIOS boot. For advanced configuration options including custom boot flags, see xref:Kiwi_Appliance_Support_and_UEFI_Boot_SHE_cli_deploy[Kiwi Appliance Support and UEFI Boot Automation].
