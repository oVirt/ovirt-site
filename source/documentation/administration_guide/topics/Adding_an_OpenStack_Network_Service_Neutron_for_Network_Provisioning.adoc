...
.. If the OpenStack Networking instance uses SSL, the *Import provider certificates* window opens; click *OK* to import the certificate that the OpenStack Networking instance provides to ensure the {engine-name} can communicate with the instance.
