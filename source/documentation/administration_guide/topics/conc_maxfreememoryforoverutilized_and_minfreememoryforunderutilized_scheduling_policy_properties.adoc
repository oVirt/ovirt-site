...
* To prevent the host from over utilization of all the physical CPUs, define the virtual CPU to physical CPU ratio - *VCpuToPhysicalCpuRatio* with a value between 0.1 and 2.9. When this parameter is set, hosts with a lower CPU utilization are preferred when scheduling a virtual machine.
+
