...
The upload action will allow a user to upload a file from his/her local machine to the oVirt environment. Once a file is selected and the upload process is initiated, WebAdmin will stream the data to the servlet (possibly on a different machine than the engine), which will send the data to vdsm as it is received, which in turn will write it to the storage. During the upload process the user will be updated via a progress bar, and will have the ability to cancel an ongoing upload. Pausing and/or resuming uploads may also be supported.
...
Auth will be handled by a set of tickets generated by the engine. In response to the initial request to engine, engine will generate a pair of tickets: one will be to authenticate the UI connection to the servlet, and the other will be for vdsm's use. Both tickets will also contain data relevant to the upload process: the first will direct the servlet to the appropriate vdsm, while the second contains information about the target file.
...
If connection issues occur, the UI code should issue limited retries to ease use of flaky connections. If this is not adequate or other failures occur, the partial upload will remain on the storage. The user can then choose to attempt to resume the upload or cancel it altogether (thus removing the data) if desired. During the upload process, the temporary data should be accounted for in space utilization queries, quotas, etc as appropriate. (TODO: determine which places this is needed)
