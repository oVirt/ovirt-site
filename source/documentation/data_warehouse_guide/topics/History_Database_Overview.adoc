...
{virt-product-fullname} {engine-name} uses `PostgreSQL 10.x` as a database platform to store information about the state of the virtualization environment, its configuration and performance. At install time, {virt-product-fullname} {engine-name} creates a PostgreSQL database called `engine`.
