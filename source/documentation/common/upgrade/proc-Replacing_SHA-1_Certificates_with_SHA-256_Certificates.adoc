...
[WARNING]
====
Do _NOT_ let certificates expire because once expired, the environment becomes non-operational and recovery is an error prone and time consuming process. For information on renewing certificates, see link:{URL_virt_product_docs}{URL_format}administration_guide/index#chap-Renewing-certificates_{context}[Renewing certificates before they expire] in the _Administration Guide_.
====
