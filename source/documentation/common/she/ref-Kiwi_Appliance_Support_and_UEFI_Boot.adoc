...
The default appliance package name now follows the pattern `ovirt-engine-appliance-<distro><major_version>` (for example, `ovirt-engine-appliance-centos9` or `ovirt-engine-appliance-almalinux9`), automatically matching your host operating system. If the distribution-specific package is not available, it falls back to the CentOS-based package.
