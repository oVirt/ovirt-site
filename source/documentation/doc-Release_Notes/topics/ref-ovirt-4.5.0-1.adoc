...
A link to the Monitoring Portal has been added to the AdministYou can set the number of PCI Express ports for VMs with `engine-config`.ration Portal dashboard.
...
You can now use the Logical Volume Management (LVM) devices file for managing storage devices instead of LVM filter, which can be complicated to set up and difficult to manage. Starting with RHEL 9, this will be the default for storage device management.
