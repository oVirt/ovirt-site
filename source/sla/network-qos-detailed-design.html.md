...
The newly added part will only be visible only to user with the permission to edit QoS properties, both in Administrator and User portal.

**UI addition to the Add/Edit VNIC dialog**
![](QoS.png "fig:QoS.png")

In the Add/Edit Network two parts will be added: - QoS properties of the network(discussed in the next phase) - QoS properties preset for VNICs attached to the network (discussed here).
...
Once inbound/outbound was enabled all three field must be filled (This will be verified before allowing to close the dialog). If QoS properties were set in the network dialog as preset for all VNICs attached to that network - these setting will appear in the VNIC. The user can override them in the VNIC dialog.
