...
The next phase of live merge is a polling phase conducted by the engine. As long as the VDSM getAllVmStats API reports a job with a matching jobID, the job is assumed to be ongoing and engine just waits. VDSM is careful to save a record of the block job in the virtual machine recovery file so that polling will maintain consistency over VDSM restarts. Since the block job is part of the qemu process, any action that causes a virtual machine to down will abort all block jobs for that virtual machine. Once the job is no longer reported, engine enters the final phase of the job which resolves what happened and cleans up.
