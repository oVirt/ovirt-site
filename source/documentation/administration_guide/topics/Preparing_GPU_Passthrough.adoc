...
For more information on the hardware requirements for direct device assignment, see link:{URL_downstream_virt_product_docs}html/planning_and_prerequisites_guide/rhv_requirements#PCI_Device_Requirements_RHV_planning[PCI Device Requirements] in the _Planning and Prerequisites Guide_.
...
The host GPU can now be directly assigned to the prepared virtual machine. For more information on assigning host devices to virtual machines, see link:{URL_virt_product_docs}virtual_machine_management_guide[Host Devices] in the _Virtual Machine Management Guide_.
