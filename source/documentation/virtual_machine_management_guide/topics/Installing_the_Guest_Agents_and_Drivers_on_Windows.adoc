...
. Upload [filename]`virtio-win___version__.iso` to a data storage domain. See link:{URL_virt_product_docs}administration_guide/index#Uploading_Images_to_a_Data_Storage_Domain_storage_tasks[Uploading Images to a Data Storage Domain] in the __Administration Guide__ for details.
