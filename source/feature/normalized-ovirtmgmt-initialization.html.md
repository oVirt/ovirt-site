...
| New 3.0 host installation                                                | Add host to a 3.0 cluster                                                                       | Host should show up in Engine (with management network as bridge)                                   |          |                                              |
...
| New ovirt-node installation (ovirtmgmt is a VM network)                  | Add ovirt-node to a 3.1 system                                                                  | Host should show up in Engine, ovirtmgmt on host is a VM network.                                   |          |                                              |
...
| Negative: New host installation with non-responsive VDSM                 | Add host to cluster 3.1 and above, after Termination message appears, shutdown VDSM on the host | Installation failed, host in Non-Responsive status, event log informs error communicating with VDSM |          |                                              |
