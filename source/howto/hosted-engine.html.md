...
2.  Upgrade hosts with new packages(changes repository to 3.4 and run yum update -y) on this stage appear vdsm-tool exception <https://bugzilla.redhat.com/show_bug.cgi?id=1088805>
...
6.  Enter for example via UI to engine and change 'Default' cluster(where all your hosted hosts seats) compatibility version to 3.4 and activate you hosts(because after vdsm upgrade they in non-operational state)
