...
`then ovirt repo -   sudo yum localinstall `[`http://resources.ovirt.org/releases/ovirt-release-el6-10.0.1-2.noarch.rpm`](http://resources.ovirt.org/releases/ovirt-release-el6-10.0.1-2.noarch.rpm)
...
*   A Fedora 19 node goes to "non operational" state with the error "Host IPADDRESS is compatible with versions (3.0,3.1,3.2,3.3) and cannot join Cluster Default which is set to version 3.4.".

The recommended fix is to enable the fedora-virt-preview repo, delivered with the ovirt-release , and to update libvirt from it.
