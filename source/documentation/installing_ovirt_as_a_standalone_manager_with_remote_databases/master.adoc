...
. The {virt-product-fullname} {engine-name} must run on {enterprise-linux} 8. For detailed installation instructions, see link:{URL_rhel_docs_latest}html/performing_a_standard_rhel_installation/index.html[_Performing a standard {enterprise-linux-shortname} installation_].
+
...
To install the {virt-product-fullname} {engine-name} on a system that does not have access to the Content Delivery Network, see xref:Configuring_an_Offline_Repository_for_Red_Hat_Virtualization_Manager_Installation_SM_remoteDB_deploy[Configuring an Offline Repository for Installation] before configuring the {engine-name}.
