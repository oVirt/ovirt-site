...
You can migrate the Data Warehouse service installed and configured on the {virt-product-fullname} {engine-name} to a separate machine. Hosting the Data Warehouse service on a separate machine helps to reduce the load on the {engine-name} machine. Notice that this procedure migrates the Data Warehouse service only. To migrate the Data Warehouse database (`ovirt_engine_history`) prior to migrating the Data Warehouse service, see xref:Migrating_the_Data_Warehouse_Database_to_a_Separate_Machine_{context}[].
...
** The username and password for the Data Warehouse database from the {engine-name}'s */etc/ovirt-engine-dwh/ovirt-engine-dwhd.conf.d/10-setup-database.conf* file. If you migrated the `ovirt_engine_history` database using xref:Migrating_the_Data_Warehouse_Database_to_a_Separate_Machine_{context}[], the backup includes these credentials, which you defined during the database setup on that machine.
...
Enable the {virt-product-fullname} repositories and install the Data Warehouse setup package on a {enterprise-linux} 7 machine:
...
----
+
. If the database is hosted on a remote machine, you must manually grant access by editing the postgres.conf file. Edit the _/var/opt/rh/rh-postgresql10/lib/pgsql/data/postgresql.conf_ file and modify the listen_addresses line so that it matches the following:
+
[options="nowrap" subs="normal"]
----
listen_addresses = '*'
...
+
See xref:Migrating_the_Data_Warehouse_Database_to_a_Separate_Machine_{context}[] for more information on how to configure and migrate the Data Warehouse database.
...
xref:Changing_the_Data_Warehouse_Sampling_Scale[].
====
endif::DWH[]

[[disabling_dwh_package]]
