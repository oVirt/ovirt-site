...
* `Dedicated` - the selected CPU policy is available for cluster compatibility level 4.7 or later. If the virtual machine has NUMA enabled, all nodes must be unpinned.
* `Isolate Threads` - the selected CPU Policy is available for cluster compatibility level 4.7 or later. If the virtual machine has NUMA enabled, all nodes must be unpinned.
. Click btn:[OK].
