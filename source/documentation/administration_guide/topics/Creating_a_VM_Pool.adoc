...
You can create a virtual machine pool containing multiple virtual machines based on a common template.
// See link:{URL_virt_product_docs}vmm-guide/Virtual_Machine_Management_Guide.html#chap-Templates[Templates] in the _Virtual Machine Management Guide_ for information about sealing a virtual machine and creating a template.
...
If you need to configure different `sysprep` settings for different pools of Windows virtual machines, you can create a custom `sysprep` file in the Administration Portal (see link:#custom_sysprep[Creating a Virtual Machine Pool] below).
//  See link:{URL_virt_product_docs}vmm-guide/Virtual_Machine_Management_Guide.html#Using_Sysprep_to_Automate_the_Configuration_of_Virtual_Machines[Using Sysprep to Automate the Configuration of Virtual Machines] in the _Virtual Machine Guide_ for more information.
...
. Check that the Windows virtual machine has an attached floppy device with the unattend file, for example, `A:\Unattend.xml`.
...
. Save the registry, seal the Windows virtual machine, and create a new template. 
====
// See link:{URL_virt_product_docs}vmm-guide/Virtual_Machine_Management_Guide.html#chap-Templates[Templates] in the _Virtual Machine Management Guide_ for details.
