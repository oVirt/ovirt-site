...
* Select *Don't Change* to make no changes to the virtual machine.

* Select *Existing* to use the existing CPU topology that has been set, in order to configure CPU pinning and NUMA pinning.

* Select *Adjust* to maximize the CPU topology and generate the CPU pinning and NUMA pinning configurations.
|No
