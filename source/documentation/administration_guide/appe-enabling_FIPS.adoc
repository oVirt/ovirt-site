...
[id="enabling-fips_in_ovirt"]
= Enabling FIPS in {virt-product-shortname}
...
[IMPORTANT]
====
Red Hat recommends installing {enterprise-linux-shortname} with FIPS mode enabled as opposed to enabling FIPS mode later. Enabling FIPS mode during the installation ensures that the system generates all keys with FIPS-approved algorithms and continuous monitoring tests in place.
====
...
* xref:enabling_fips_el_hosts_and_ovirt_appendix_fips[Enabling FIPS in {enterprise-linux-shortname} hosts and the standalone Manager]

include::topics/proc-enabling-fips_she.adoc[leveloffset=+1]

include::topics/proc-enabling-fips_el_hosts_and_ovirt.adoc[leveloffset=+1]

.Additional resources

* link:{URL_downstream_virt_product_docs}installing_ovirt_as_a_self-hosted_engine_using_the_command_line/index#Installing_Red_Hat_Virtualization_Hosts_SHE_cli_deploy[Installing {virt-product-fullname} Hosts]
