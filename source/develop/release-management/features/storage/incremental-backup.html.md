...
- Perform a full backup of entire disk without using temporary
  snapshots, for both raw and qcow2 disks.

- Perform incremental backup for disks using qcow2 format without
  temporary snapshots.
...
1. If backup application specified a cleanup checkpoint, engine deletes
   all checkpoints older than the specified checkpoint using vdsm
   checkpoints API. vdsm uses libvirt checkpoints APIs to perform the
   request.
...
When backup fail or aborted, backup application did completed to copy
all data.
...
XXX Missing way to specify checkpoint cleanup after backup.
