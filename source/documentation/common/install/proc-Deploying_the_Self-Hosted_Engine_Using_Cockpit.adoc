...
* For Fibre Channel:
.. Enter the *LUN ID*. The host bus adapters must be configured and connected, and the LUN must not contain any existing data. To reuse an existing LUN, see link:{URL_virt_product_docs}administration_guide/index.html[Reusing LUNs] in the _Administration Guide_.
.. Enter the *Disk Size (GiB)*.
