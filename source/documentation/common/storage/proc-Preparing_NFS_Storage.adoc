...
Set up NFS shares on your file storage or remote server to serve as storage domains on {virt-product-fullname} hosts systems.
After exporting the shares on the remote storage and configuring them in the {virt-product-fullname} {engine-name}, the shares will be automatically imported on the {virt-product-fullname} hosts.
