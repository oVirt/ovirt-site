...
{org-fullname} recommends updating one cluster at a time.

//Limitations
include::snip-Host_update_limitations.adoc[]

ifdef::4-0_local_db,4-0_remote_db,4-0_SHE[]
* {hypervisor-shortname} 4.0 hosts cannot be updated with {virt-product-fullname} {engine-name} 4.3. They must be updated manually from the command line:
+
...
----
+
This limitation applies only to {hypervisor-shortname} 4.0. Other {hypervisor-shortname} versions and all {enterprise-linux-host-shortname} can be updated using {virt-product-fullname} {engine-name} 4.3.
endif::4-0_local_db,4-0_remote_db,4-0_SHE[]
