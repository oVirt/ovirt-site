...
[NOTE]
====
If upgrading from version 4.4.9 to a later version fails on RHV-H, run the `dnf reinstall redhat-virtualization-host-image-update` command to fix the issue.
====
endif::rhv-doc[]
...
[NOTE]
====
If upgrading from version 4.4.9 to a later version fails on RHV-H, run the `dnf reinstall ovirt-node-ng-image-update` command to fix the issue.
====
endif::ovirt-doc[]
