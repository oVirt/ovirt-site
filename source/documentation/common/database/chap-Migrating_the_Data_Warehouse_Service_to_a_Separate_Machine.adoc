...
To migrate the Data Warehouse database (`ovirt_engine_history`) prior to migrating the Data Warehouse service, see xref:chap-Migrating_Data_Warehouse_Database_to_a_Separate_Machine.adoc#Migrating_the_Data_Warehouse_Database_to_a_Separate_Machine[Migrating the Data Warehouse Database to a Separate Machine].
...
+
If you migrated the `ovirt_engine_history` database using the procedures described in xref:chap-Migrating_Data_Warehouse_Database_to_a_Separate_Machine.adoc#Migrating_the_Data_Warehouse_Database_to_a_Separate_Machine[Migrating the Data Warehouse Database to a Separate Machine], the backup includes these credentials, which you defined during the database setup on that machine.
...
include::proc-setting-up-the-new-data-warehouse-machine.adoc[leveloffset=+1]
include::proc-stopping-the-data-warehouse-service.adoc[leveloffset=+1]
include::proc-configuring-the-new-data-warehouse-machine.adoc[leveloffset=+1]
include::proc-disabling-the-data-warehouse-service.adoc[leveloffset=+1]
